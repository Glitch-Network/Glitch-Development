...
localStorage.setItem("in_bounds",0) // DO NOT REMOVE THIS LINE, ITS ESSENTIAL FOR THE ARROW SYSTEM
localStorage.setItem("new_instance","0") // DO NOT REMOVE THIS LINE, ITS ESSENTIAL FOR THE ARROW SYSTEM

// if not in a iframe
if (window.top === window.self) { // if not in an iframe

    setTimeout(function(){
        if (localStorage.getItem("in_bounds") == "0" && (window.location.href).includes("main.html") == false) { // if not inbounds and not in main.html
...
if (localStorage.getItem("ai_teacher_detection") === "true" && localStorage.getItem("premium") === "true") {
    ...
    document.body.onload = () => {
        ...
        script.onload = init; // Initialize the application after TensorFlow.js is loaded
        document.head.appendChild(script);
    };

    function init() {
        // Show a button to start the webcam and AI detection
        const startButton = document.createElement('button');
        startButton.innerText = "Start AI Detection";
        startButton.onclick = main; // Main function starts on button click
